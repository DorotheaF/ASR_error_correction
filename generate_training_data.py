...
def generate_error_thinking(transcript):
    ...
    for i in range(3, length_transcript-3):
        ...
        transcript.iloc[i,11] = response[0]
...
def format_as_dataset(transcript):
    prompt_style = """
    ### Instruction:
    This is an excerpt from a discussion between tutors and students in a zoom classroom. The transcript was automatically
    ...
    mistakes that were originally made by the student or tutor, but CORRECT errors created by the speech recognition system.
    Here are 7 lines from the transcript. Look at the middle utterance and 1) print the utterance, 2) indicate if the utterance
    has one or more errors introduced by the speech recognition system, and 3) if there are introduced errors, 
    what is the corrected sentence. Several lines of context are provided to help decide if
    ...
    for i in range(3, length_transcript - 3):
        ...
        thinking = transcript.iloc[i]

        if utterance == edited_utterance:
            errors = "There are no errors introduced by the ASR system"


        input_text = prompt_style.format(pre_context, utterance, post_context, utterance, edited_utterance, "")
...
def cycle_through_transcripts(location, save_path):
    ...
    for file in transcripts:
        ...
        transcript_formatted = format_as_dataset(transcript)
        prompt_list = prompt_list + transcript_formatted
